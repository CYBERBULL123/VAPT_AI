...
def render_main_app():
    ...
    if tabs == "🛡️ VAPT":
        ...
        st.markdown("""
            ...
            |-----------------------------------------------|-------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
            | 🔴 **Heatmap: Severity vs Exploitation Likelihood** | **Critical Zones:** High-severity vulnerabilities align with the highest exploitation likelihood. **Hidden Risks:** Medium-severity findings can pose overlooked dangers. | 🚨 **Action Needed:** Attackers prioritize high-likelihood vulnerabilities for exploitation. 🛠️ **Mitigation:** Patch high-severity issues immediately. |
            | 📊 **Bar Chart: Risk Scores by Severity**     | **Risk Leaders:** High-severity issues dominate total risk scores.Medium-severity also contributes significantly. | 🎯 **Focus Areas:** Attackers exploit high-risk contributors first. 📈 **Resource Allocation:** Prioritize mitigation strategies for top risk scores. |
            | 📝 **Pie Chart: Findings by Severity**        | **Threat Breakdown:** High-severity findings are fewer but far more critical. **Majority Share:** Medium/low-severity dominate in count. | 🔍 **Focus Shift:** High-severity issues are attacker favorites. 🚧 **Preventative Measures:** Address medium/low-severity to block lateral movements. |
            | 📉 **Scatter Plot: Risk vs Exploitation Likelihood** | **Critical Clusters:** High-risk, high-likelihood findings form key target zones. **Subtle Threats:** Low-likelihood risks can evolve. | 🚩 **Primary Target:** Attackers exploit clustered vulnerabilities for maximum damage. 🛡️ **Strategic Defense:** Minimize critical clusters proactively. |
            | 📈 **Line Chart: Exploitation Likelihood Over Findings** | **Trending Risks:** Consistently high-likelihood findings highlight recurring vulnerabilities. Fluctuating risks need constant monitoring. | 📅 **Audit Timelines:** Track trends to avoid recurring risks. 🌟 **Proactive Defense:** Predict and address upcoming attack trends effectively. |
            | 🟦 **Histogram: Distribution of Risk Scores** | **Risk Patterns:** Majority of findings fall within moderate risk ranges (e.g., 60–80). Outliers (>90) are urgent concerns. | 🔥 **Priority Fixes:** Attackers target high-risk outliers for significant impacts. 🚀 **Quick Wins:** Addressing outliers reduces the attack surface. |
